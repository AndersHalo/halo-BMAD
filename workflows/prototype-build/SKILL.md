...
description: Generate a multi-screen HTML prototype from a PRD with mock data, a floating State panel for role and scenario switching, functional interactive widgets, and role-based navigation. Use when building a clickable proof-of-concept from a product requirements document.
---
...
Reads a PRD from the project directory, analyzes it, and generates a multi-file HTML prototype — one HTML file per screen — with embedded mock data, a floating State panel for role and scenario switching, functional interactive widgets in each screen, and role-based navigation. Stack: HTML + CSS + Vanilla JS. One `.html` file per screen. No build step. Output lands in `{project-root}/{workflow.output_dir}/{prototype-name}/`.
...
- `{output_dir}/{prototype-name}/.prototype-session.json` is the active session file — its presence indicates a prior run for this prototype.
- Output files are written to `{project-root}/{workflow.output_dir}/{prototype-name}/` (resolved from PRD filename in Find and Read PRD).
...
4. Load reference files (eager) — read `references/schema-instructions.md`, `references/style-guide.md`, and `references/interactive-components.md` in full. If any cannot be read: ⚠ Could not read `{filename}` at `{skill-root}`. Check the path and try again. Load `references/control-panel.md` and `references/session-tracking.md` lazily just before Generate begins.
...
Read the file at `prd_path`. Store as `prd_content`.
...
Allow the user to mix numbers and paths in one reply. Read each confirmed file. Store as `aux_docs[]` — each entry: `{ path, label: filename, content }`. Total aux content may be large; read all requested files before proceeding.
...
**Format detection** — apply extraction hints during Analyze PRD:
...
- `output_dir`: `{project-root}/{workflow.output_dir}/{prototype_name}/`
...
- **2 / Patch** → ask:
  ...
  Accept free-text, file path(s), or both. Read any provided files. Store as `patch_description`. `run_mode = "patch"`. Proceed.

If `prior_session = null`: `run_mode = "full"`, proceed silently.

## Analyze PRD

Silently analyze `prd_content` **and all `aux_docs[]` content** (using format hints from Find and Read PRD) and extract. Treat aux docs as additive to the PRD — roles, screens, FRs, and interactions defined in aux docs are merged into the same analysis. When the same FR appears in both, the aux doc's version takes precedence.
...
Store as `fr_list[]`. If no explicit FR labels exist, derive one entry per distinct feature or capability described. Aux doc FRs are always included. Include `FRs identified: {N} ({M} from aux docs)` in the analysis summary.
...
Store `patch_targets`. Steps 4–7 run in full; Generate limits file writes to `patch_targets` only.
...
3. If neither found: use defaults from `references/style-guide.md`
...
If the user provides a styleguide file path: read it and extract tokens.

## Schema

Using the rules in `references/schema-instructions.md` and the confirmed screens from Analyze PRD, silently derive a UI-adapted data schema.
...
Cross-reference every entity against every state and role confirmed in Analyze PRD. Rules:
...
```js
{
  key:               "{screen-slug}_{story-slug}",
  label:             "{Descriptive story name}",
  role:              "{role}",
  frs:               ["{FR-id}", ...],
...
**Lazy-load (if not already in context):** read `references/control-panel.md` and `references/session-tracking.md` in full.

**Pre-generation check:** If any screen, flow, or component lacks enough detail to generate (missing field types, ambiguous role restrictions, undefined interaction), list the gaps and wait for answers before continuing. For each interactive component type needed, reference the matching pattern in `references/interactive-components.md`. If nothing is missing, proceed immediately.
...
<head>
  ...
  <style>
    ...
    /* State panel styles — see references/control-panel.md */
    /* Screen-specific styles */
  </style>
</head>
<body>

  <!-- FLOATING STATE PANEL — follows references/control-panel.md rules exactly -->

  <!-- SCREEN CONTENT -->
  <div id="screen-content"></div>

  <script>
    /* ── SCREEN DATA — keyed by scenario (from Data phase B) ── */
    const SCREEN_DATA = { ... }
    ...
    /* ── STATE PANEL — from references/control-panel.md ── */
...
`login.html` — State panel shows Scenarios section only (no Role override). Each scenario represents logging in as a specific role. Login card contains only form fields and submit button.
...
- **Locked / restricted screens:** render passive text only:
  ```html
...
If issues found: fix inline and list the fixes. If issues cannot be auto-fixed: ask:
...
Write tracking documents following the exact formats in `references/session-tracking.md`:
...
- `{output_dir}/.prototype-session.json` — always fully overwritten; `files_written` lists all written screen files; includes `snapshot`, `last_run`, and `last_increment` fields; `snapshot.fr_list[]` stores extracted FRs from Analyze PRD; `snapshot.scenarios[]` stores confirmed scenarios from Data; `last_run.aux_docs[]` stores the filenames of all aux docs read
...
- **y** →
  ...
  3. Write `{output_dir}/docs/increments/increment-{N}-{date}.md` per format in `references/session-tracking.md`
