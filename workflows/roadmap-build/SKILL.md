---
name: roadmap-build
description: Generate a product roadmap from a PRD with optional architecture and sprint context into Now/Next/Later, Theme-Based, OKR, and Gantt formats with AI-agent effort estimates. Use when creating or updating a product roadmap from existing requirements.
---
...
Generate a product roadmap from your PRD — with optional architecture, feature specs, and sprint context — into one or more HTML formats with AI-agent effort estimates. Produces Now/Next/Later, Theme-Based, Goal-Oriented (OKR), and Gantt Chart formats. Supports resume from checkpoint and sprint-status refresh without re-reading documents.
...
- `{skill-root}/session.json` is the checkpoint file — `step` field drives resume routing.
- Output files are written to `{workflow.output_dir}/{product_name}/` (resolved after PRD is read in Collect and Read Documents).
...
4. Load reference files — read `references/estimation-model.md`, `references/roadmap-conventions.md`, `references/team-roles-reference.md`, and `references/sprint-parsing.md` in full. If any cannot be read, stop with: ⚠ Missing reference file: `{filename}` at `{skill-root}`. The skill may be corrupted — try reinstalling.
5. Load `{project-root}/_bmad/core/config.yaml` (and `config.user.yaml` if present); resolve `{user_name}`, `{communication_language}`. Missing → neutral defaults; never block.
6. **Team governance** — try to read `{workflow.governance_path}`. If found, extract `team_profile` and normalize all role names to standard keys using `references/team-roles-reference.md`. Any unrecognized role is stored as `other_role` and treated as Senior Developer baseline for estimation. If not found, apply solo-developer defaults silently and show: > ℹ No governance file found — estimates will use solo-developer defaults. Run `team-governance` to configure your team profile.
7. **Detect mode.** **Headless** when: no TTY, programmatic caller, `activation_steps_prepend` declares headless, or first message pre-supplies all required inputs. If headless: load `references/headless.md` and follow it for the entire run.
8. **Session check** — try to read `{skill-root}/session.json`. If found and valid JSON, parse the `step` field.

   **If `step` is a number (active session):**
   ```
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ...
   **If `step` is `"done"` (completed roadmap):**
   ```
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ...
   - **Refresh** → proceed to **Refresh**.
   - **Iterate** → delete `{skill-root}/session.json` and proceed to Choose Format.
   - **Fresh** → delete `{skill-root}/session.json` and proceed normally.
...
Triggered from the session done-checkpoint banner (On Activation step 8) or from the Done completion menu.

Load format-specific reference files needed for this run — read `references/format-now-next-later.md`, `references/format-theme-based.md`, `references/format-goal-oriented.md`, `references/format-gantt.md` as applicable based on `format_selection` from the done checkpoint. If `output_mode` was `"both"`, also read `references/format-html.md`.

**Target roadmap** — if multiple product folders exist in `{workflow.output_dir}/`, list them and ask:
> Which roadmap do you want to refresh? (number or name)

Otherwise use `output_dir` from the done checkpoint directly.

**Read existing roadmap files** — read existing roadmap files from `output_dir`. Check for HTML files first (`roadmap-*.html`); if none exist, fall back to markdown files (`roadmap-*.md`). Extract:
...
- **y** → re-read from stored path.
- **path / new path** → ask for path, then read.
- **skip** → proceed without updating sprint status.

If read successfully, parse per `references/sprint-parsing.md` rules, then re-run the sprint matching and status derivation logic from Analyze Documents against the extracted initiative list. Recompute `any_sprint_coverage`, `sprint_status_counts`, and `sprint_coverage[initiative]` for all initiatives.
...
- Update sprint status columns, hero Sprint Status stat, and Effort Summary Sprint Progress using recomputed coverage data and `references/roadmap-conventions.md` rules.
...
Display the same completion summary as Done. Update the done checkpoint.
...
Load format-specific reference files:

| format_selection | Files to load |
|---|---|
| 1 | `references/format-now-next-later.md` |
| 2 | `references/format-theme-based.md` |
| 3 | `references/format-goal-oriented.md` |
| 4 | `references/format-gantt.md` |
| 5 | all four format files |

If `output_mode` is `"both"`, also load `references/format-html.md`.
...
**User-initiated format switch** — if the user requests to change format at any point during steps 1–4, update `format_selection`, immediately load the new format file(s), and if the new format includes Gantt (4 or 5) and `gantt_start` is null, ask for it before proceeding.
...
Read the file. If it cannot be read, re-ask — up to 3 attempts:
...
> Output → `{workflow.output_dir}/{product_name}/`
...
Resolve `output_dir` as `{workflow.output_dir}/{product_name}/` using the sanitized name.
...
If provided:
- **Single path** → read the file.
- **Comma-separated paths** → split on `,`, trim whitespace, read each.
- **Directory path** → scan for all `.md` files in that directory (non-recursive), read each.
...
If provided, read. If unreadable:
> ⚠ Could not read `{path}` — skipping.

> Sprint / task board file path? *(Jira CSV or markdown table — leave blank to skip)*

If provided, read and parse the task list per `references/sprint-parsing.md` rules. Store as:
```
sprint_tasks = [
...
Sprint groups are matched to PRD initiatives in Analyze Documents, after the initiative list is known.
...
Display:
> ✓ Output: `{output_dir}`{if current_version: " · v{current_version} found"}
...
Using the initiative list just extracted, match each sprint task group (`epic` field from `sprint_tasks`) to an initiative using layered fuzzy matching. Apply in order until a match is found:
...
3. Semantic similarity — e.g. `authentication` matches `Auth`, `resource-directory` matches `Resource Directory Page`
...
**Unmatched sprint groups** — after processing all initiatives, identify sprint groups that could not be matched. If any exist, display:
> ⚠ Unmatched sprint groups: `{group1}`, `{group2}`, ...
> Continue without these groups, or provide manual mappings?

- **Continue** → exclude those groups and proceed.
...
## Estimate All Initiatives

Using `references/estimation-model.md`, assign for every initiative:
...
Convert critical-path cycle counts to calendar weeks using the Calendar Time Conversion table in `references/estimation-model.md`. Use week ranges as time labels — do NOT use quarters or month ranges.
...
Apply all sprint status display, hero stat block, and Effort Summary rules from `references/roadmap-conventions.md`. Format-specific embedding of story cycles and effort lines is defined in each `references/format-*.md` file.
...
1. If any individual format files were not loaded in Choose Format, load them now.
...
When generating `roadmap-view.html`, check whether `{output_dir}/roadmap-changelog.md` exists. If it does, read it and include a Changelog section per the spec in `references/format-html.md`.
...
**Versioning** — only if `current_version` was found in Collect and Read Documents:
...
- **1 (Iterate)** → return to Collect and Read Documents and reload documents fresh. Keep team profile in memory.
- **2 (New format)** → return to Choose Format. Keep loaded documents and estimates in memory.
- **3 (Refresh)** → proceed to **Refresh** using documents already in memory.
- **4 (Done)** → execute `{workflow.on_complete}`. Invoke `bmad-help` to surface next-skill suggestions.
